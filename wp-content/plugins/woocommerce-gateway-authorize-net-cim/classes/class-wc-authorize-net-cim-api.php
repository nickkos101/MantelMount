...
class WC_Authorize_Net_CIM_API_Request extends XMLWriter {
	...
	public function get_transaction_request_xml( $order ) {
		...
		foreach ( $order->get_items() as $item_id => $item ) {
			...
			$line_items[] = array(
				'item_id'     => $item_id,
				'name'        => $item['name'],
				'description' => trim( is_object( $product ) ? $product->get_sku() . ' ' . $product->get_title() : '') . ' ' . $meta,
				'quantity'    => $item['qty'],
		...
		foreach ( $order->get_fees() as $fee_id => $fee ) {
			...
			$line_items[] = array(
				...
				'quantity'    => 1,
				'unit_price'  => $order->get_item_total( $fee ),
			);
		}
	...
	private function add_taxes( WC_Order $order ) {
		...
		// <amount>
		$this->writeElement( 'amount', number_format( $order->get_total_tax(), 2, '.', '' ) );
	...
	private function add_shipping( WC_Order $order ) {
		...
		// <amount>
		$this->writeElement( 'amount', number_format( SV_WC_Plugin_Compatibility::get_total_shipping( $order ), 2, '.', '' ) );

		// <name>
		$this->writeElement( 'name', __( 'Shipping', WC_Authorize_Net_CIM::TEXT_DOMAIN ) );

		// <description>
		$this->writeElement( 'description', substr( $order->get_shipping_method(), 0, 255 ) );
