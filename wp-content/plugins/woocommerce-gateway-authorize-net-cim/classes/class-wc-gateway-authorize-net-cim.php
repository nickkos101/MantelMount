		...
		if ( $this->icon ) {
			...
			foreach ( $this->card_types as $card_type ) {
				...
				if ( is_readable( $wc_authorize_net_cim->get_plugin_path() . '/assets/images/card-' . strtolower( $card_type ) . '.png' ) ) {
					$icon .= '<img src="' . esc_url( SV_WC_Plugin_Compatibility::force_https_url( $wc_authorize_net_cim->get_plugin_url() ) . '/assets/images/card-' . strtolower( $card_type ) . '.png' ) . '" alt="' . esc_attr( strtolower( $card_type ) ) . '" />';
				}
			}
		}
